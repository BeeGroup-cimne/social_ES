...
def EducationAndEmploymentCensus(wd, municipality_code=None, years=None, mode="relative"):
    ...
    for pc in tqdm(province_codes, desc="Downloading data from INE..."):
        filename = f"{path}/{pc-1}_{mode}.parquet"
        ...
        if not os.path.exists(filename):
            ...
            agg_cols = [
                ...
                if col not in ["Country code", "Province code", "Municipality code", "District code", "Section code",
                               "Year"]
            ]
...
def EssentialCharacteristicsOfPopulationAndHouseholds(
    ...
    path = path_creator(path, wd)
    filenames = {
        "variables_meta": f"{path}/variables_meta.pkl",
        "filtering_variables": f"{path}/filtering_variables.pkl",
        "dfs": f"{path}/dfs.pkl"
    }
    ...
    if not all([os.path.exists(filename) for filename in filenames.values()]):
        ...
        for k in dfs.keys():
            for i in range(len(dfs[k])):
                ...
                if "~".join(subclasses) not in variables_split:
    ...
    atlas_df = HouseholdIncomeDistributionAtlas(wd=wd)['Sections']
    atlas_df = atlas_df[atlas_df.Year==2021]
    population_df = PopulationCensus(wd=wd)['Sections']
                ...
                if len(filt_vars_)>0:
                    ...
                    if ((cases_df_[pivot_df_vars].values == 0) | (cases_df_[pivot_df_vars].values == 1)).all():
                        ...
                        for municipio, group in pivot_df.groupby("Municipios"):
                            ...
                            try:
                                b = group[target]
                                x, residuals, rank, s = np.linalg.lstsq(A.values, b.astype(float).values, rcond=None)
